...
def calculate_metric_score(real_labels, predict_score):
    ...
    print(
        '************************AUPR score:%.3f,  AUC score:%.3f, f score:%.3f,accuracy:%.3f, precision score:%.3f, recall score:%.3f************************' % (
    ...
    for CV in range(0, CV_num):
        ...
        for i in range(0, len(trainPosition)):
            if method_num == 1:
                # 1: Average
                X_train.append(
                    (model.vectors[str(trainPosition[i, 0] + 1)] + model.vectors[str(trainPosition[i, 1] + 1)]) / 2)
            elif method_num == 2:
                # 2: Hadamard
                X_train.append(
                    model.vectors[str(trainPosition[i, 0] + 1)] * model.vectors[str(trainPosition[i, 1] + 1)])
            elif method_num == 3:
                # 3: Weighted-L1
                X_train.append(
                    np.fabs(model.vectors[str(trainPosition[i, 0] + 1)] - model.vectors[str(trainPosition[i, 1] + 1)]))
            elif method_num == 4:
                # 4: Concatenate
                X_train.append(np.concatenate(
                    (model.vectors[str(trainPosition[i, 0] + 1)], model.vectors[str(trainPosition[i, 1] + 1)]), axis=0))
        ...
        for i in range(0, len(testPosition)):
            if method_num == 1:
                # 1: Average
                X_test.append(
                    (model.vectors[str(testPosition[i, 0] + 1)] + model.vectors[str(testPosition[i, 1] + 1)]) / 2)
            elif method_num == 2:
                # 2: Hadamard
                ...
                # 3: Weighted-L1
                X_test.append(
                    np.fabs(model.vectors[str(testPosition[i, 0] + 1)] - model.vectors[str(testPosition[i, 1] + 1)]))
            elif method_num == 4:
                # 4: Concatenate
                X_test.append(
                    np.concatenate(
                        (model.vectors[str(testPosition[i, 0] + 1)], model.vectors[str(testPosition[i, 1] + 1)]),
                        axis=0))
...
CV_num, method_num, alpha, beta, gama, mu, dimension, drug_size, removed_ratio, learning_rate, pu = [3, 4, 0.1, 2, 0.1,
                                                                                                     1e-5, 128, 2367, 0,
                                                                                                     0.001, 1]
...
results = cross_validation(adj, CV_num, method_num, alpha, beta, gama, mu, dimension, drug_size, learning_rate, pu)
print(results)
